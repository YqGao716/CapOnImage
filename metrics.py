...
def diversity(data_pred):
  ...
  return np.mean(div1), np.mean(div2)

def split_token(sents):
  for i in range(len(sents)):
    sents[i] = ' '.join([c for c in sents[i]])
  return sents
...
def compute(preds, refs, names):
  ...
  for i in range(len(preds)):
    candcaps[str(i)] = split_token([preds[i]])
    refcaps[str(i)] = split_token([refs[names[i]]])
